...
const { mcpWebSearch, getAliMcpQuotaState, MCP_ENDPOINT, BAIDU_ENDPOINT } = require('./webSearchMcp'); // 阿里百炼(一次性2000次,本地计数保险) / 百度千帆(每日100次)
...
let gSearchMode = 'builtin';
let gVolcKey = '';
let gVolcModel = '';
let gBaiduKey = '';
function setSearchMode(mode) {
  gSearchMode = normSearchMode(mode);
}
function setSearchCreds(c) {
  gVolcKey = (c && c.volcApiKey) || '';
  gVolcModel = (c && c.volcModel) || '';
  gBaiduKey = (c && c.baiduApiKey) || '';
}
...
const SEARCH_CHANNELS = {
  ...
  volc: {
    ...
    getKey: () => gVolcKey,
    run: (key, query) => volcWebSearch(key, query, { model: gVolcModel || DEFAULT_VOLC_MODEL }),
  },
  baidu: {
...
async function callLLM(provider, apiKey, model, messages, opts) {
      ...
      try {
        ...
        if (mxr && mxr.ok && mxr.text && mxr.text.length >= 120) {
          ...
          if (wantMxQuery) {
            ...
            return await postLLM(p.url, apiKey, mxBody);
          }
  ...
  const channel = SEARCH_CHANNELS[gSearchMode];
  if (channel && webSearch && apiKey) {
    const searchKey = channel.getKey(apiKey);
    if (!searchKey) {
      console.warn(`[search:${gSearchMode}] 未配置该通道的 API Key，本次回退内置 enable_search`);
    } else {
      try {
        ...
        if (query) {
          ...
          if (results && results.trim()) {
            ...
            return await postLLM(p.url, apiKey, body);
          }
        }
      } catch (searchErr) {
        console.warn(`[search:${gSearchMode}] 调用失败，回退内置 enable_search：`, searchErr && searchErr.message);
        // 落入下方内置路径
      }
    }
  }
  ...
  try {
    ...
    if (hasSearchParam && e.response && e.response.status === 400) {
      ...
      return await postLLM(p.url, apiKey, retryBody);
    }
    throw e;
  }
}
...
async function postLLM(url, apiKey, body) {
...
async function buildLocalEarningsContext(symbol, opts = {}) {
  ...
  try {
    ...
    // 已上传财报清单（资料库）
    let docs = [];
    try { docs = docStore.listCompanyDocuments(String(symbol)) || []; } catch (e) { docs = []; }
...
// 估值大模型（提示词驱动）：读取 prompts/valuation-system.md 作为系统提示词，联网获取实时数据后输出结构化估值
async function analyzeValuation({ symbol, stockName, industry, force, companyName }) {
  ...
  const messages = [
    { role: 'system', content: systemPrompt },
    { role: 'user', content: `请对以下 A 股上市公司做估值分析：\n公司：${name || symbol}（代码 ${symbol}${ind ? '，行业：' + ind : ''}）。\n\n请优先使用本工作台的「上网搜索」获取最新行情、可比公司估值与宏观利率，并尽量引用本工作台「年报数据库」（已上传的历年财报）中的历史财务。严格按你的系统提示词中的"输出模板"给出结构化结论（内在价值区间 / 当前判断 / 安全边际 / 主要风险），并在每条数据后标注来源与日期。` },
  ];
  try {
    const content = await callLLM(cfg.provider, cfg.apiKey, modelPick.model, messages, { webSearch: true });
    const result = {
      symbol,
      stockName: name || symbol,
      industry: ind || '',
