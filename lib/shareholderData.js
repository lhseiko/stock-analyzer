 ...
 *   - 十大股东 / 十大流通股东 (RPT_F10_EH_HOLDERS，仅取季度末报告期)
...
// 报告期判定：仅认季度末（03-31 / 06-30 / 09-30 / 12-31）。
// 东财 RPT_F10_EH_HOLDERS 会混入「临时公告日」的少量行（如长江电力 2026-08-22 仅 1 行、
// 海天味业 2026-07-03），若直接取 max(END_DATE) 会拿到非报告期的残缺记录，
// 表现为「十大股东只剩 1 条」或「最新报告期不是 6-30」——即用户反馈的「东财有 6-30 数据但工作台获取不到」。
...
async function fetchTopShareholders(emCode) {
  ...
  try {
    ...
    const rows = data?.result?.data;
    if (!Array.isArray(rows) || rows.length === 0) return [];
    // 只认「季度末报告期」的最新一档，避免取到临时公告日（如 2026-08-22 仅 1 行）导致十大股东残缺
    const qDates = [...new Set(rows.map(r => (r.END_DATE || '').slice(0, 10)).filter(isQuarterEnd))].sort();
    let latestDate = qDates.length ? qDates[qDates.length - 1] : pickLatestReportDate(rows.map(r => (r.END_DATE || '').slice(0, 10)));
    let periodRows = rows.filter(r => (r.END_DATE || '').slice(0, 10) === latestDate);
    // 极端兜底：该报告期记录不足 10 条时，向前回退到最近一个满 10 条的季度末
    for (let i = qDates.length - 2; i >= 0 && periodRows.length < 10; i--) {
      const cand = rows.filter(r => (r.END_DATE || '').slice(0, 10) === qDates[i]);
      if (cand.length >= 10) { latestDate = qDates[i]; periodRows = cand; }
    }
