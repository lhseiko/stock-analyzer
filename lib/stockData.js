...
async function getMarketOverview() {
  ...
  if (panelIsEastmoney) {
    panelNote = '东方财富·行业板块口径（申万细分行业，含地面兵装等细分板块），与东方财富行情软件板块榜一致';
  } else if (panelSource === '同花顺·行业板块') {
    panelNote = '分类口径为同花顺申万一级行业，粒度比东方财富「行业板块」粗（地面兵装等细分板块并入军工装备，不单列）';
  } else {
...
async function getEastmoneySectorRanking() {
  ...
  // 东方财富「行业板块」现为申万多层级口径，同一主题会同时存在「XXⅡ / XXⅢ」两条（数值相同）。
  // 按基础名去重（保留绝对值较大的那条，并统一用基础名展示），避免涨幅前五出现重复项（如地面兵装Ⅱ/Ⅲ）。
  const seen = new Map();
  ...
  const result = {
    sectorsUp: upList.slice(0, 5),
    sectorsDown: downList.slice(0, 5),
    allSectors: list,             // 全量板块（供板块资金流向做申万层级兜底映射）
...
module.exports = { detectMarket, getQuote, getHistory, getHistoryPeriod, getHistoryDeep, getEastmoneyTurnover, searchStocks, fetchTencentMinutes, fetchEastmoneyHistory, getEastmoneySecid, getMarketOverview, getEastmoneySectorRanking, parseSwSectorName, findPythonForScript };
