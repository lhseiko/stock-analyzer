...
const { getEastmoneySectorRanking, parseSwSectorName } = require('./stockData');
...
async function fetchAll() {
  ...
  // 用东方财富行业板块全量榜兜底补全申万层级（资金流接口本身不带层级后缀的条目）
  try {
    const emSectors = await getEastmoneySectorRanking();
    const levelMap = new Map();
    if (Array.isArray(emSectors.allSectors)) {
      for (const s of emSectors.allSectors) {
        if (s.swLevel && s.name) levelMap.set(s.name, s.swLevel);
      }
    }
    for (const list of [result.todayInflowTop5, result.todayOutflowTop5, result.retailInflowTop5, result.retailOutflowTop5,
                       result.fiveDayMaxInflow, result.fiveDayMaxOutflow, result.retailFiveDayMaxInflow, result.retailFiveDayMaxOutflow]) {
      if (!Array.isArray(list)) continue;
      for (const it of list) {
        if (it && !it.swLevel && levelMap.has(it.name)) it.swLevel = levelMap.get(it.name);
      }
    }
  } catch (e) {
    // 兜底失败不影响主数据
    console.warn('[SectorCapitalFlow] 申万层级兜底失败:', e.message);
  }
