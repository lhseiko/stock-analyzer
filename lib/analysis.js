...
function evaluateSignals(fundamentals, companyType, industryAvg) {
  ...
  if (pe < 0) {
    ...
    if (pe < peAvg * 0.8) signals.push({ key: 'pe', label: '市盈率 PE(TTM)', value: pe.toFixed(1) + ' 倍', signal: 'bull', reason: `PE(TTM) ${pe.toFixed(1)} 低于行业均值 ${peAvg.toFixed(2)}（约 ${(pe / peAvg * 100).toFixed(0)}%），估值偏低，利好` });
    else if (pe > peAvg * 1.2) signals.push({ key: 'pe', label: '市盈率 PE(TTM)', value: pe.toFixed(1) + ' 倍', signal: 'bear', reason: `PE(TTM) ${pe.toFixed(1)} 高于行业均值 ${peAvg.toFixed(2)}（约 ${(pe / peAvg * 100).toFixed(0)}%），估值偏贵，利空` });
    else signals.push({ key: 'pe', label: '市盈率 PE(TTM)', value: pe.toFixed(1) + ' 倍', signal: 'neutral', reason: `PE(TTM) ${pe.toFixed(1)} 接近行业均值 ${peAvg.toFixed(2)}` });
  }
