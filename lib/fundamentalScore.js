...
//   ② 盈利能力：优先取本地财报库近两三年季/半年/年报，按最新披露的滚动 12 个月（TTM）口径
//      计算 ROE / 净利率 / 毛利率；均值高于行业中枢得满分，低于则按与中枢的差值比例扣分。
...
// ---------- 财报库：滚动 12 个月（TTM）取值 ----------
// zyzbHistory：东财主要财务指标多期序列（含 REPORT_DATE / REPORT_TYPE，累计口径）。
// TTM = 最新期(M月累计) + 上年年报 − 上年同期(M月累计)；最新期为年报时直接取该值。
function ttmFromReports(hist, field) {
  if (!Array.isArray(hist) || !hist.length) return { value: null, basis: '', reason: '本地财报库无多期数据' };
  const rows = hist
    ...
    .sort((a, b) => b.d - a.d);
  if (!rows.length) return { value: null, basis: '', reason: `财报库无「${field}」字段` };
  const latest = rows[0];
  const M = latest.d.getMonth() + 1;          // 3/6/9/12
  if (M === 12) return { value: latest.v, basis: `${latest.name}（年报直取）`, reason: '' };
  const y = latest.d.getFullYear();
  const lastAnnual = rows.find(r => r.d.getFullYear() === y - 1 && r.d.getMonth() + 1 === 12);
  const sameLastYear = rows.find(r => r.d.getFullYear() === y - 1 && r.d.getMonth() + 1 === M);
  if (!lastAnnual || !sameLastYear) {
    return { value: null, basis: '', reason: `缺上年报或上年同期数据，无法计算滚动12个月「${field}」` };
  }
  const v = latest.v + lastAnnual.v - sameLastYear.v;
  return { value: v, basis: `${latest.name} + ${lastAnnual.name} − ${sameLastYear.name}（滚动12个月）`, reason: '' };
}
...
function computeFundamentalScore({ quote, companyType, industryAvg } = {}) {
  ...
  // ================= ② 盈利能力（25 分） =================
  const ttmRoe = ttmFromReports(zy, 'ROEJQ');
  const ttmNet = ttmFromReports(zy, 'XSJLL');
  const ttmGross = ttmFromReports(zy, 'XSMLL');
  ...
  result.dimensionLogic.profitability = {
    ...
    summary: `盈利能力维度固定 ${DIM} 分；按最新披露滚动 12 个月口径计算 ROE / 净利率 / 毛利率，与行业中枢比较，高于得满分、低于按比例扣分；无法取值项不参与评分。本次得 ${profActual}/${DIM} 分。`,
  };
...
module.exports = { computeFundamentalScore, ttmFromReports, yoyPair, percentileRank, industryPercentile, buildBenchmark };
