...
function factorHoldings(shareholders, buyback) {
  ...
  return {
    key: 'holdings', name: '增持减持', weight: 0.043, signal, applicable: true, value, detail, subFactors: subs,
    // 20260913f：因子级数据日期/来源标注（折叠态也能看到数据报告期，不再"小卡片无日期"）
    caption: `数据报告期：${periodTag} · 来源：东方财富 F10 前十大股东（含前十大流通股东）`,
  };
}
