 ...
 *   1) 市场情绪与消息面（网络讨论热度 + 个股/板块新闻情感 + 财报是否符合预期 + 内幕抢跑 + 板块消息联动 + 全网舆情；
 *      20260909c 起「跨市场传导」子卡删除、「量能活跃度」子卡迁移至资金量能）
 *      —— 20260823i 起，原「公司近期重大利好/利空」因子已并入本因子（权重由 0.16 提升至 0.38）
 *   2) 资金量能          (capitalFlow 主力净流入 + 量价 + 融资余额变化 + 换手率变化 + 量能活跃度[20260909c 迁入])
...
function factorSentiment(sentiment, quote, history, sectorNews, crossMarket, globalSentiment, newsParts, symbol, capital, turnoverChange) {
  const parts = [];
  ...
  // 3) 舆情与讨论热度（个股）—— Part B 升级为「单只A股个股全网舆情分析」确定性模块（lib/stockSentiment）。
  //    消费：同花顺讨论帖(stockDiscussion) + 东财/同花顺/雪球热度(discussionHeat) + 个股新闻(newsSentiment)，
  //    并按行情/主力资金/换手率做四路交叉校验 → 综合分(0-100) + 热度等级 + 情绪标签 + 告警 + 利好/中性/利空。
  //    保留旧逻辑（修复 ns.ok 判空 bug）作为兜底：buildStockSentiment 不可用时卡片仍可展示基础舆情。
  let stockSent = null;
  try {
  ...
  if (stockSent && stockSent.ok) {
    ...
    parts.push({
      name: '舆情与讨论热度（个股）', w: 0.30, signal: clamp(stockSent.signal, -1, 1),
    ...
    if (hasNews || hasHeat) {
      ...
      // 修复：ns 可能为 {ok:false}（如 curl_cffi 受限），必须以 ns.ok 判空，否则渲染 undefined
  ...
  if (sectorNews && sectorNews.ok && typeof sectorNews.signal === 'number') {
    ...
    const evtTxt = (sn.eventBoosted && sn.events && sn.events.length)
      ...
      : '';
    parts.push({
      name: '板块消息', w: 0.20, signal: clamp(sn.signal, -1, 1),
  ...
  // 6) 跨市场传导：20260909c 起按用户要求全个股删除该子卡片（数据获取与 crossMarketMeta 经验学习保留，仅不再展示/计分）
  ...
  // 8) 量能活跃度：20260909c 起迁移至「资金量能」因子（量能属资金面范畴，与 20260823g 融资余额迁移同理）
...
function factorMarketShort(market, sector, stockContext) {
  const picked = (stockContext && stockContext.selectedIndices) || []; // 已按真实成分股筛好的指数
  ...
  // 首页大盘技术分析模块的短期研判（看多/看空/震荡），按个股真实成分筛后的指数，
  // 求平均信号作为「研判共振」。例：士兰微 → 上证指数 + 沪深300 三个方向均值。
  ...
  // 20260905e：合并"大盘指数"与"首页大盘技术分析"两个重复逻辑的子因子 → 仅保留"大盘短期走势"
  const detail = `大盘：${scopeLabel} 平均 ${cnAvg >= 0 ? '+' : ''}${round(cnAvg)}%（${cnAvg > 0.3 ? '偏强' : cnAvg < -0.3 ? '偏弱' : '中性'}）；大盘短期走势研判：${techVerdict}（${techDetail}）`;
  const subs = [
    {
      name: '大盘短期走势',
      signal: clamp(techSigAvg, -1, 1),
      value: techVerdict,
      detail: techPicks.length
        ? `${scopeLabel} 短期研判：${techDetail} → 综合 ${techVerdict}；该股市场口径平均涨跌 ${cnAvg >= 0 ? '+' : ''}${round(cnAvg)}%`
        : '暂未提供与该股同市场的短期研判，按中性处理',
    },
  ];
  if (secOk) {
    subs[0].detail += `；行业：${sector.boardName || sector.industryName} ${secChg >= 0 ? '+' : ''}${round(secChg)}%（涨 ${up}/跌 ${down}）`;
  ...
  const value = `研判 ${techVerdict} / 行业 ${secOk ? (secChg >= 0 ? '+' : '') + round(secChg) + '%' : '—'}`;
...
function computeNewsFactor(news, quote, history, report, symbol, marketSentiment) {
  ...
  // 20260906b：缓存 promptVersion 过期（stale=true）时不引用其旧文——避免提示词升级后
  // 行情判断小卡继续吐旧版全文快照；此时回退 scraper 评级分支，待财报解读重新生成后自动恢复联动。
  ...
  if (deepSummaryOk) {
    ...
    // 20260906c：小卡只引用精简结论（verdict 一句话），详细正文只在深度分析卡展示（用户明确分工：小卡=精简、深度卡=详细）
    const earnPart = head + (verdictTxt ? '；' + verdictTxt : '');
...
function factorSeesaw(seesaw, isTechStock) {
  if (isTechStock) {
    return {
      key: 'seesaw', name: '科技指数（负相关）', weight: 0.056, signal: 0, applicable: false,
      value: '不适用',
  ...
  if (!seesaw || !seesaw.ok) {
    return {
      key: 'seesaw', name: '科技指数（负相关）', weight: 0.056, signal: 0, applicable: true,
      value: '—',
  ...
  return {
    key: 'seesaw', name: '科技指数（负相关）', weight: 0.056,
...
function factorTechnicalShort(pa) {
  ...
  if (!stx || typeof stx.dirScore !== 'number') {
    return {
      key: 'technicalShort', name: '个股短期动向', weight: 0.200,
      signal: 0, applicable: true, value: '—',
      detail: '个股短期动向数据不足（K线<60 或价格行为推演未返回短期动向），按中性处理。来源：价格行为趋势推演（日/60分钟级别），与「技术面分析」页同源。',
      subFactors: [{ name: '短期动向预判', signal: 0, value: '—', detail: '短期数据不足，预判缺失。来源：价格行为趋势推演' }],
    };
  }
  ...
  // 20260905g：卡片只展示「结论」（原始 7 档方向，与技术面分析页②完全同口径），证据链不进 UI。
  // 主 detail 保留一句话来源说明（同一次快照，可追溯）。
  const detail = `个股短期动向（预判：${dirTxt}）：与技术面分析页②短期动向预判同一次快照、同一档位展示（lib/priceAction.shortTerm.direction，唯一出口 priceActionHub），概率 ${stx.probability || '—'}。`;
  const subs = [
    {
      name: '短期动向预判',
      signal: dirSig,
...
const FACTOR_NAME = {
  ...
  sectorLimit: '板块涨跌停占比', seesaw: '科技指数（负相关）',
...
const LAYOUT_VERSION = '20260910c'; // 展示布局/口径调整时手动 +1；因子结构变化由哈希自动捕获（20260910c：子卡「板块消息联动」改名「板块消息」，显示名变更手动升版强制重算；结算复盘按双名兼容旧记录）
...
async function buildJudgment(symbol, name, industry) {
  ...
  const sectorTrend = await getSectorTrend(symbol, name || '', fineIndustry || industryName).catch(() => null);

  // 板块涨跌停占比：个股所属板块内涨停/跌停家数占比（短期情绪核心因子）
  const sectorLimit = await getSectorLimitStats(symbol, name || '', fineIndustry || industryName).catch(() => null);
  ...
  const factors = [
    factorSentiment(sentiment, quote, history, sectorNews, crossMarket, globalSentiment, newsSubs.parts, symbol, capital, turnoverChange),
    factorCapital(capital, turnoverChange, sentiment, quote, history),
    factorFuturesShort(futures),
    factorMarketShort(market, sectorTrend, stockCtx),
  ...
  // 20260907a：第三遍叠加「事件驱动」权重覆盖。
  // 既有 8 因子按各自自适应权重等比例压缩，新增「事件驱动」因子，总和恒为 100%。
  // 无活跃事件时 evShort 为 null → 行为与旧版完全一致（weightsUsed = adaptive）。
  const evShort = eventEngine.buildEventOverride({ symbol, baseWeights: adaptive, factorKeys: FACTOR_KEYS, horizon: 'short' });
  const finalFactors = evShort ? factors.concat(evShort.eventFactor) : factors;
  const finalWeights = evShort ? evShort.override : adaptive;
...
async function _refreshTechnicalShortOnReuse(rec, symbol) {
  try {
    ...
    const newFactor = factorTechnicalShort(paSnap);
