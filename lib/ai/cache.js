...
const EARNINGS_PROMPT_VERSION = '20260909i'; // 20260909i：中报环比口径改为「Q2单季 vs Q1单季」（严禁「上半年累计 vs 上一年年报」跨口径比较）；强制逐条「已核实/未核实」来源标注；雪球/股吧/论坛/自媒体降级（禁止作为数字依据）；注入系统确定性核算的单季事实块。20260906c：恢复详细 8 维度输出（用户分工：深度分析卡=详细解读），保留 ≤1000 字精简提炼；行情判断小卡改为只引用 verdict 精简结论（sameDayJudgment 侧）。20260906b（4 行极简版）废弃。20260906a：取消 800 字上限。20260904c：修正 reportPeriod 提取 + summary 无条件剥离「报告期/标的」行；联网 AI 主路径 + 身份确认（防串公司）保持不变
