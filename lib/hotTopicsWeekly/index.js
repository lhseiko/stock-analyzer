...
function buildFootnotes(cfg, meta, sources) {
  const notes = [
    ...
    `指标口径：A=归属快讯条数（代理曝光量）；B=社区讨论帖数增量+采样帖浏览评论和（同花顺讨论 API，按板块代表性成分股归集）；C=韭菜句式帖数；D/E=看多占比%。`,
  ];
  if (meta.degradeMode) {
    notes.push('⚠️ 降级模式：社区讨论通道（同花顺讨论 API）受限，B/E 指标缺失，权重已重分配为 A×0.8+D×0.2。');
  }
  if (sources.guba && sources.guba !== 'ok') {
    notes.push(`社区讨论当日状态：${sources.guba}（${sources.gubaBoards} 板块成功）；社区讨论统一取自同花顺讨论 API（按板块代表性成分股归集），不再扫描东财股吧（规避验证码墙）。`);
  }
  return notes;
}
